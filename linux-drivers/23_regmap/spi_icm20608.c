...
struct ICM20608
{
    ...
    int gyro_x_adc;  // 陀螺仪X轴原始值
    int gyro_y_adc;  // 陀螺仪Y轴原始值
    int gyro_z_adc;  // 陀螺仪Z轴原始值
    int accel_x_adc; // 加速度计X轴原始值
    int accel_y_adc; // 加速度计Y轴原始值
    int accel_z_adc; // 加速度计Z轴原始值
    int temp_adc;    // 温度原始值
...
void icm20608_read_data(struct ICM20608 *icm20608)
{
    ...
    regmap_bulk_read(icm20608->regmap, ICM20_ACCEL_XOUT_H, data, 14);
