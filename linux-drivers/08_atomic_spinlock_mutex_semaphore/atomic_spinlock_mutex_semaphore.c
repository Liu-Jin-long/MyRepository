...
{
#ifdef ATOMIC
    if (!atomic_dec_and_test(&LED_GPIO.count))
    {
        atomic_inc(&LED_GPIO.count);
        return -EBUSY;
    }
#endif
#ifdef SPINLOCK
    unsigned long flags;
    spin_lock_irqsave(&LED_GPIO.spinlock, flags);
    if (LED_GPIO.device_status == 0)
    {
        spin_unlock_irqrestore(&LED_GPIO.spinlock, flags);
        return -EBUSY;
    }
    LED_GPIO.device_status--;
    spin_unlock_irqrestore(&LED_GPIO.spinlock, flags);
#endif

#ifdef MUTEX
#if MUTEX_INT
    if (mutex_lock_interruptible(&LED_GPIO.mutex))
    {
        return -ERESTARTSYS;
    }
#else
    mutex_lock(&LED_GPIO.mutex);
#endif
#endif

#ifdef SEMAPHORE
    if (down_interruptible(&LED_GPIO.semaphore))
    {
        return -ERESTARTSYS;
    }
#endif
...
int LED_GPIO_close(struct inode *inode, struct file *f)
{
    ...
    // struct LED_GPIO *dev = f->private_data;
#ifdef ATOMIC
    atomic_set(&LED_GPIO.count, 1);
#endif
...
#ifdef MUTEX
    mutex_unlock(&LED_GPIO.mutex);
#endif

#ifdef SEMAPHORE
    up(&LED_GPIO.semaphore);
#endif
...
#endif
#ifdef SPINLOCK
    spin_lock_init(&LED_GPIO.spinlock);
    LED_GPIO.device_status = 1;
#endif
#ifdef MUTEX
    mutex_init(&LED_GPIO.mutex);
#endif
#ifdef SEMAPHORE
    sema_init(&LED_GPIO.semaphore, 1);
#endif
