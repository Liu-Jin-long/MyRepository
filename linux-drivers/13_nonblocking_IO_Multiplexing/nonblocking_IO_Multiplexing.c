...
// #define INTERRUPTIBLE // 等待事件被挂起可中断
// #define UNINTERRUPTIBLE // 等待事件被挂起不可中断
#define YIELD // 事件未到，主动让出CPU挂起，可自定状态
//--------------------
#define USER_FD_NONBLOCK // 应用程序使用的文件描述符设置了非阻塞模式
...
static ssize_t iMX6ULL_IRQ_DEV_read(struct file *f, char __user *buf, size_t count, loff_t *pos)
{
...
#ifdef USER_FD_NONBLOCK
    if (f->f_flags & O_NONBLOCK)
    {
        if (atomic_read(&piMX6ULL_IRQ_dev->key_status) != KEY_PUSH_STATE)
        {
            return -EAGAIN;
        }
    }
...
#ifdef YIELD
        add_wait_queue(&piMX6ULL_IRQ_dev->read_wait_queue_head, &wait);
    ...
    {
        // printk("驱动 status == KEY_PUSH_STATE\n");
        // printk("驱动 value == %#X\n", value);
...
static unsigned int iMX6ULL_IRQ_DEV_poll(struct file *f, struct poll_table_struct *poll_table)
{
    unsigned int mask = 0;
    struct iMX6ULL_IRQ_DEV *piMX6ULL_IRQ_dev = (struct iMX6ULL_IRQ_DEV *)f->private_data;
    poll_wait(f, &piMX6ULL_IRQ_dev->read_wait_queue_head, poll_table); // 将当前内核驱动线程放入wait_queue，不会阻塞
    if (atomic_read(&piMX6ULL_IRQ_dev->key_status) == KEY_PUSH_STATE)
    {
