...
static void timer_func(unsigned long parg)
{
    ...
    if (value == 0) // 按下动作消抖结束-->按键按下
    {
        ...
        if (piMX6ULL_IRQ_dev->async_queue)
        {
            // 发送信号给应用进程（发送的信号需要手动注销）
            kill_fasync(&piMX6ULL_IRQ_dev->async_queue, SIGIO, POLL_IN);
        }
    }
    else // 松开动作消抖结束-->按键松开
    {
...
int iMX6ULL_IRQ_DEV_fasync(int fd, struct file *f, int on)
{
    struct iMX6ULL_IRQ_DEV *piMX6ULL_IRQ_dev = (struct iMX6ULL_IRQ_DEV *)f->private_data;
    return fasync_helper(fd, f, on, &piMX6ULL_IRQ_dev->async_queue); // 从相关的进程列表中添加或去除entry项
}
int iMX6ULL_IRQ_DEV_close(struct inode *inode, struct file *f)
{
    int ret = iMX6ULL_IRQ_DEV_fasync(-1, f, 0);
    return ret;
}
