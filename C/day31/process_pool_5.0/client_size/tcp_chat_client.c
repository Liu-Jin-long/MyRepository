...
int main(int argc,char** argv)
{
    ...
    off_t file_size,download_size=0,last_size=0;//download_size是当前大小,last_size是上次大小
    recv_n(sfd,&file_size,data_len);

    off_t slice_size=file_size/10000;//0.01%
    while (1)
    {
        ...
        if(data_len>0)
        {
            ...
            if(download_size-last_size>slice_size)
            {
